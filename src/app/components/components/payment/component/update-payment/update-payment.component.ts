...
export class UpdatePaymentComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.itemDetailsForm.setValue({
      pid: this.data.pidFk?.pid,
      paymentDate: this.data.pidFk?.paymentDate,
      // status: this.data.status,
      paymentStatus: this.data.pidFk?.paymentStatus,
      price: this.data.pidFk?.price
    });
  ...
  approveDialogError4(){
    this.dialog.open(ApprovalDialogComponent, {
      width: '350px',
      // height: '200px',
      data: new ApprovalDialogConfig('Fine', 'Successfull!', 'Payment Status Updated')
    });
  }
